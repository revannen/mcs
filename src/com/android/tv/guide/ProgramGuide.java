...
public class ProgramGuide implements ProgramGrid.ChildFocusListener {
    ...
    private final FrameLayout mHideWidget;
...
//        }
        ...
        //for mcs
        mCurrentTime = mContainer.findViewById(R.id.program_guide_header_currenttime);
        ...
        timeHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                String tmp = format.format(current);
                tmp = tmp.toLowerCase();
                mCurrentTime.setText(tmp.substring(0, tmp.length()-1));
                timeHandler.postDelayed(this, 1000);
            }
        }, 10);

        mHideWidget = (FrameLayout) mContainer.findViewById(R.id.program_guide_side_panel_hide);
    }
...
//                }
                ...
                if (mShowGuidePartial) {
                    ...
                    if(mHideWidget.isShown()) {
                        mHideWidget.setVisibility(View.GONE);
                    }
                } else {
                    mShowAnimatorFull.start();
                    if(!mHideWidget.isShown()) {
                        mHideWidget.setVisibility(View.VISIBLE);
                    }
                }
            }
        };
...
//        mHandler.postDelayed(mHideRunnable, mShowDurationMillis);
    ...
    private void updateGuidePosition() {
        ...
        if (tableHeight > screenHeight) {
            ...
            mTable.setPaddingRelative(startPadding, 0, 0, 0);
            LayoutParams layoutParams = mTable.getLayoutParams();
            layoutParams.height = LayoutParams.WRAP_CONTENT;
            mTable.setLayoutParams(layoutParams);
        } else {
            mTable.setPaddingRelative(startPadding, 0, 0, bottomPadding);
            LayoutParams layoutParams = mTable.getLayoutParams();
...
//                    offset - mCurrentTimeIndicatorWidth / 2, 0, 0, 0);
            RelativeLayout.MarginLayoutParams mp = new RelativeLayout.MarginLayoutParams(mCurrentTimeIndicator.getLayoutParams());
            mp.setMargins(offset - mCurrentTimeIndicatorWidth / 2 + res.getDimensionPixelOffset(R.dimen.program_guide_table_header_column_width), -8, 0, 0);
            RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(mp);
            mCurrentTimeIndicator.setLayoutParams(layoutParams);
